...
| Podcast transcripts | Target `kruse-archive/podcasts/<year>/<month>/*.md`; owned Jack Kruse YouTube channel videos use `kruse-archive/podcasts/<year>/<month>/youtube-channel/*.md`; current legacy artifacts under `summary/kruse-summary/out/issue-225*/transcripts/` | `media_items`, `kruse_podcast_transcripts`, `kruse-archive/podcasts/` | Included when archive markdown exists through the podcast transcript bundles | Backend daily runner for owned-channel new-only refresh; issue-specific import workers for the legacy/general backlog | Legacy issue transcript artifacts are not yet all in the canonical archive path or live Supabase import; general podcast transcript import is not fully daily-owned |
